...
CONFIGS_DIR = "configs"  # Local path in the repo
...
def channel_active_today(config: dict) -> bool:
    """Check if this channel publishes today based on upload_days."""
...
def generate_topic(config: dict) -> str:
    """Call Claude API to generate a compelling video topic for tonight."""
...
def main():
    ...
    for channel_id in channels_to_run:
        ...
        if not channel_active_today(config):
